...
- The port accepted years written `" 2025"`, `"+2025"` or `"02025"`, rows from
  another dataflow, and, with ANSI mode off, a 20-digit head count that became
  `Long.MaxValue`. All three are now refused, as R refuses them.
- The port's second hash of the extract ran while bronze was still a lazy read
  of the file, so later reads were unchecked. Bronze is now read once, into a
  checkpoint, before the second hash, and the local check asserts it.
- The table assertions in `R/checks.R` (one published total per class-year,
  every class-year tiered, every year covered) had no tests; deleting any one of
  them left CI green. They now live in `R/tables.R` and `tests/test-r-tables.R`
  feeds each a broken fixture. The `recent_above` test used data for which the
  first and last four years give the same answer.
- `R/checks.R` and the tests now stop with "run from the repository root"
  rather than "cannot open the connection" when run from elsewhere.
- The CI version check now also requires `CITATION.cff`, the CHANGELOG and, on a
  release, the report to agree on the date, and a pushed tag to name the
  version.

### Changed

- Hand-typed years and counts in the report are computed: the suppression
  chart's title, the number of livestock codes, the year Canterbury drops out
  of the top three, the census-year `S` comparison and the perturbation year
  (a named constant, cited). The subtitle is checked against the window.
- The README check counts how many times each phrase appears, and covers the
  numbers in the image description, the imputation rate and the class-year
  count. The dairy net is written in millions, like the rise and fall around
  it; the README says what each dairy denominator counts and what a class-year
  is.
- Output CSVs write a missing value as an empty field, as the extract does,
  rather than `NA`; rows are sorted by the numeric area code; and
  `dairy-comparison-windows.csv` is renamed `class-comparison-windows.csv`,
  since it holds all three classes. janitor is no longer a dependency.
- The port derives its window from a `dim_year` table written once by the
  ingest notebook, asserts that 2014 is still the dairy peak, and stamps silver,
  quarantine, the rule summary and every gold table with a `run_id` parameter;
  gold tables also carry the source hash.
- The site can be redeployed from a manual workflow run on `main`.
- pip, setuptools and wheel, which build pyspark from source, are pinned and
  hashed in `databricks/build-requirements.txt` and used with
  `--no-build-isolation`. Both lockfiles are generated by uv, and their `.in`
  files now say so.
- `data-raw/SOURCE.md` cites the 2024 release the verification totals came from,
  and uses the report's names for the two flags.
- The Databricks dashboard screenshot, which showed since-corrected output, is
  removed; the run-graph screenshots have the workspace paths and job IDs
  masked.
